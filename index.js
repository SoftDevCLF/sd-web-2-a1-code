...
function ageFilter(array, age) {
  ...
  const filterAgeAndNameValidation = array.filter(
    (user) => age > user.age && user.name
  );
  if (filterAgeAndNameValidation.length === 0) {
    return `<li>No characters in the list  were found below the specified age.</li>`;
  }
  return filterAgeAndNameValidation
...
renderAgeFilteredList.innerHTML =
  ageFilter(users, 25) + ageFilter(characters, 25);
